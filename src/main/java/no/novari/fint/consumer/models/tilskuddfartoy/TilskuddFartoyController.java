package no.novari.fint.consumer.models.tilskuddfartoy;
...
import no.novari.fint.model.resource.arkiv.kulturminnevern.TilskuddFartoyResources;
...
public class TilskuddFartoyController {
    ...
    public TilskuddFartoyResources getTilskuddFartoy(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getTilskuddFartoyByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("TilskuddFartoy cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public TilskuddFartoyResources getTilskuddFartoyByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getTilskuddFartoy(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private TilskuddFartoyResources getTilskuddFartoyByOdataFilter(
        ...
        Event event = new Event(
                ...
                KulturminnevernActions.GET_TILSKUDDFARTOY, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new TilskuddFartoyResources();
    
        ArrayList<TilskuddFartoyResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<TilskuddFartoyResource>>() {});
